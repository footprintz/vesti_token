 ...
 * behavior.
 */
contract VestiPresale is Ownable{
   ...
   */
  constructor(uint256 _rate, address _tokenAddress, address payable wallet) public {
    require(_rate > 0);
    require(_tokenAddress != address(0));

    // set the presale rate
    rate = _rate;
  ...
  function buyTokens(address payable _beneficiary) public payable {
    ...
    // if tokens amount is more than available token for presale
    ...
    internal
  {
    ...
    require(!(isDistributionFinished), "Presale has ended.");
    require(token.balanceOf(address(this)) > 0, "No tokens for presale");
  }
  ...
  function _processPurchase(address _beneficiary, uint256 _tokenAmount) internal{
        require(_tokenAmount <= amountForPresale());
  ...
  function amountForPresale() public view returns(uint256){
      return amountDistributed + token.balanceOf(address(this));
  }
  ...
  function setRate(uint256 newRate) public onlyOwner{
      require(newRate > 0,"SetRate: Rate can not be zero");
      rate = newRate;
  }
  
  function setWallet(address payable newWallet) public onlyOwner{
      require(newWallet != address(0), "SetWallet: Wallet address can not be zero");
      _wallet = newWallet;
  }
